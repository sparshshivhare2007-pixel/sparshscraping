...
# Cache to store recently processed cards (with a maximum size)
processed_cards_cache = deque(maxlen=1000)
    ...
    try:
        if re.search(r'(Approved!|Charged|authenticate_successful|𝗔𝗽𝗽𝗿𝗼𝘃𝗲𝗱|APPROVED|New Cards Found By DaxxScrapper|ꕥ Extrap [☭]|み RIMURU SCRAPE by|Approved) ✅', message.text):
            ...
            for card_info in filtered_card_info:
                if card_info in processed_cards_cache:
                    continue  # Skip if card already processed
                ...
                if bin_info:
                    ...
                    # Add card info to cache to prevent duplicate sending
                    processed_cards_cache.append(card_info)
